...
static u16 sRowFirstSections[] = {
    ...
    SECTION_STONE_OF_AGONY,  SECTION_HEART_PIECES,         SECTION_DEFENSE
};
...
void KaleidoScope_DrawInventoryEditorText(Gfx** gfxP) {
    ...
    GfxPrint_Printf(&printer, "%s", T(GFXP_HIRAGANA "ﾌｳｲﾝ", "Medals"));
    GfxPrint_SetPos(&printer, 19, 19);
    GfxPrint_Printf(&printer, "%s", T(GFXP_HIRAGANA "ｾｲﾚｲｾｷ", "Stones"));
    GfxPrint_SetPos(&printer, 5, 21);
    GfxPrint_Printf(&printer, "%s", T(GFXP_KATAKANA "ｵｶﾘﾅ", "Songs"));
    GfxPrint_SetPos(&printer, 6, 24);
    GfxPrint_Printf(&printer, "%s", T(GFXP_KATAKANA "ｺﾚｸﾄ", "Misc"));
    GfxPrint_SetPos(&printer, 13, 24);
    GfxPrint_Printf(&printer, "%s", T(GFXP_KATAKANA "ｷﾝｽﾀ", "Tokens"));
    GfxPrint_SetPos(&printer, 24, 24);
    GfxPrint_Printf(&printer, "%s", T(GFXP_KATAKANA "ｶｹﾗ", "HP"));
...
void KaleidoScope_DrawInventoryEditor(PlayState* play) {
    ...
    if (CHECK_BTN_ANY(dBtnInput, BTN_DDOWN)) {
        ...
        if (--curSection < SECTION_RUPEES) {
            curSection = SECTION_MAX - 1;
        }
    } else if (CHECK_BTN_ANY(dBtnInput, BTN_DRIGHT)) {
        if (++curSection > SECTION_MAX - 1) {
    ...
    if (curSection == SECTION_RUPEES) {
        ...
        // 2 digit long box with different offset
    ...
    } else if ((curSection >= SECTION_FIRST_KEY_COUNT) && (curSection != SECTION_MAGIC)) {
        // 1 digit long box
